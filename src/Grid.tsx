...
export const VerticalGrids = (pitch: number, width: number, height: number, upperLeftX: number) => {
  const hosei = Math.ceil(upperLeftX) - upperLeftX;
  return [...(Array(Math.ceil(width / pitch)) as number[])].map((_, i) =>
...
export const HorizontalGrids = (pitch: number, width: number, height: number, upperLeftY: number) => {
  const hosei = Math.ceil(upperLeftY) - upperLeftY;
  return [...(Array(Math.floor(height / pitch)) as number[])].map((_, i) =>
